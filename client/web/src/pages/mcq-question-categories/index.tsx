...
import { getMCQQuestionsCategories } from "../../services/exam.service";
import useUserProfileStore from "../../store/user-info-store";

const MCQQuestionCategories = () => {
  ...
  const getQuestionsList = async () => {
    setLoading(true);
    try {
      const data = await getMCQQuestionsCategories();
  ...
  const getQuestions = (index: number) => {
    const path = false
      ? `/${paths.COMMUNITY_EXAMS}/banking/${paths.MCQ}/${index}`
      : `/${paths.EXAMS}/banking/${paths.MCQ}/${index}`;
    navigate(path);
  };
...
export default MCQQuestionCategories;
