...
type SubscriptionQuery struct {
	...
	withExams             *SubscriptionExamQuery
...
func (sq *SubscriptionQuery) QueryExams() *SubscriptionExamQuery {
	query := (&SubscriptionExamClient{config: sq.config}).Query()
...
func (sq *SubscriptionQuery) QueryUserSubscriptions() *UserSubscriptionQuery {
	query := (&UserSubscriptionClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) Clone() *SubscriptionQuery {
	...
	return &SubscriptionQuery{
		...
		withExams:             sq.withExams.Clone(),
		withUserSubscriptions: sq.withUserSubscriptions.Clone(),
...
func (sq *SubscriptionQuery) WithExams(opts ...func(*SubscriptionExamQuery)) *SubscriptionQuery {
	query := (&SubscriptionExamClient{config: sq.config}).Query()
...
func (sq *SubscriptionQuery) WithUserSubscriptions(opts ...func(*UserSubscriptionQuery)) *SubscriptionQuery {
	query := (&UserSubscriptionClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withUserSubscriptions = query
	return sq
}
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := sq.withExams; query != nil {
		if err := sq.loadExams(ctx, query, nodes,
			func(n *Subscription) { n.Edges.Exams = []*SubscriptionExam{} },
			func(n *Subscription, e *SubscriptionExam) { n.Edges.Exams = append(n.Edges.Exams, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withUserSubscriptions; query != nil {
		if err := sq.loadUserSubscriptions(ctx, query, nodes,
			func(n *Subscription) { n.Edges.UserSubscriptions = []*UserSubscription{} },
			func(n *Subscription, e *UserSubscription) {
...
func (sq *SubscriptionQuery) loadExams(ctx context.Context, query *SubscriptionExamQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *SubscriptionExam)) error {
	...
	query.Where(predicate.SubscriptionExam(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscription.ExamsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.subscription_exams
		if fk == nil {
			return fmt.Errorf(`foreign-key "subscription_exams" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subscription_exams" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *SubscriptionQuery) loadUserSubscriptions(ctx context.Context, query *UserSubscriptionQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *UserSubscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Subscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.UserSubscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscription.UserSubscriptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subscription_user_subscriptions
		if fk == nil {
			return fmt.Errorf(`foreign-key "subscription_user_subscriptions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subscription_user_subscriptions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
