...
func (qq *QuestionQuery) QueryExam() *ExamQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, question.ExamTable, question.ExamColumn),
		)
...
func (qq *QuestionQuery) WithExam(opts ...func(*ExamQuery)) *QuestionQuery {
	query := (&ExamClient{config: qq.config}).Query()
...
//		GroupBy(question.FieldIsActive).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := qq.withExam; query != nil {
		if err := qq.loadExam(ctx, query, nodes, nil,
			func(n *Question, e *Exam) { n.Edges.Exam = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuestionQuery) loadExam(ctx context.Context, query *ExamQuery, nodes []*Question, init func(*Question), assign func(*Question, *Exam)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Question)
	for i := range nodes {
		if nodes[i].exam_questions == nil {
			continue
		}
		fk := *nodes[i].exam_questions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "exam_questions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
