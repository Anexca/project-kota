...
func (pq *PaymentQuery) QuerySubscription() *UserSubscriptionQuery {
	query := (&UserSubscriptionClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PaymentQuery) WithSubscription(opts ...func(*UserSubscriptionQuery)) *PaymentQuery {
	query := (&UserSubscriptionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if pq.withUser != nil || pq.withSubscription != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withSubscription; query != nil {
		if err := pq.loadSubscription(ctx, query, nodes, nil,
			func(n *Payment, e *UserSubscription) { n.Edges.Subscription = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_payments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PaymentQuery) loadSubscription(ctx context.Context, query *UserSubscriptionQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *UserSubscription)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payment)
	for i := range nodes {
		if nodes[i].user_subscription_payments == nil {
			continue
		}
		fk := *nodes[i].user_subscription_payments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_subscription_payments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
